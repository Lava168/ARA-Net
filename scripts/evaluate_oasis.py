#!/usr/bin/env python3
"""Evaluate OASIS-style stress-test probability inputs."""
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("outputs/expected_results/oasis_demo_predictions.csv"))
    parser.add_argument("--metrics-json", type=Path, default=Path("outputs/expected_results/oasis_demo_metrics.json"))
    args = parser.parse_args()
    result = run_rc_spe_evaluation(args.input_csv, args.config, args.output, args.unit, args.metrics_json)
    print("[notice] OASIS should be treated as an external stress-test domain.")
